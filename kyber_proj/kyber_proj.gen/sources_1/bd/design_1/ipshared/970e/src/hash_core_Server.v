...
// Local compatibility wrappers for the four FIFO Generator IP instances used
// above. Keeping these modules in this file makes the packaged BD IP
// self-contained when Vivado does not copy the nested fifo_generator *.xci
// products into ipshared.
...
module fifo_generator_0(
	...
	fifo_generator_sync #(.WIDTH(36), .DEPTH(256), .ADDR_WIDTH(8)) u_fifo (
		.clk(clk), .srst(srst), .din(din), .wr_en(wr_en), .rd_en(rd_en),
...
module fifo_generator_1(
	...
	fifo_generator_sync #(.WIDTH(24), .DEPTH(256), .ADDR_WIDTH(8)) u_fifo (
		.clk(clk), .srst(srst), .din(din), .wr_en(wr_en), .rd_en(rd_en),
		.dout(dout), .full(full), .empty(empty)
	);
endmodule
...
module fifo_generator_7(
	...
	fifo_generator_sync #(.WIDTH(25), .DEPTH(64), .ADDR_WIDTH(6)) u_fifo (
		.clk(clk), .srst(srst), .din(din), .wr_en(wr_en), .rd_en(rd_en),
		.dout(dout), .full(full), .empty(empty)
	);
endmodule
...
module fifo_generator_8(
	...
	fifo_generator_sync #(.WIDTH(32), .DEPTH(32), .ADDR_WIDTH(5)) u_fifo (
		.clk(clk), .srst(srst), .din(din), .wr_en(wr_en), .rd_en(rd_en),
		.dout(dout), .full(full), .empty(empty)
	);
endmodule

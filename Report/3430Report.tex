...
The ZedBoard offers about 53k LUTs; operating too close to the limit worsens routing and runtime (e.g.\ $\sim$50k LUTs leading to multi-hour implementation and poor slack).
...
\noindent Official Kyber-style sampling (CBD/PRF path) consumed very large LUT in this flow (about 38k LUT in my case). An LFSR-based filler reduced \texttt{se\_gen} to about \textbf{86 LUTs} in the final routed build (with two RAMB18s for storage), at the cost of non-standard keys.
...
\noindent $\mathbf{A}$, $\mathbf{s}$, $\mathbf{e}$, $H(\mathrm{pk})$, \texttt{sk}, etc.\ are stored in \textbf{BRAM} inside modules with explicit read ports (12-bit read granularity $\Rightarrow$ software or FSM loops for full byte streams).
